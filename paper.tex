...
\item The instructor then gives learners a multiple choice question
  ...
  There is no point asking a trivial question that all students will get right or one with wrong answers which no student will pick.
  The ideal questions are those where 40--60\% of students are likely to get the right answer first time, and where every wrong answer corresponds to a misconception and will be picked by at least some students~\cite[p23]{nrc-reaching}.
...
It is also useful for motivation to make sure that students can accomplish a visible result
quickly.  Providing pre-written software libraries or source code can help to start students
closer to a meaningful result.  The idea that students must write all the code they need
themselves is the relic of a bygone era of home microcomputers (and it was not true even then).
Pick the task that you actually want to the students to engage in, and provide everything else.
...
that learners were more likely to attempt Parsons Problems than nearby multiple choice questions.
