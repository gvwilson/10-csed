...
as ``computing's most enduring and damaging myth.''  This is often ``confirmed'' by looking at university grade distributions, which are commonly held to be bimodal: a low-scoring hump of those that will never get it, and a high-scoring hump of those that have the right stuff.
...
One-on-one tutoring is perhaps the ideal form of teaching: all of a teacher's attention can be focused on one student, and they can completely customise their teaching for their single student, and tailor individual feedback and corrections based on two-way dialogue with that student.  In realistic settings however, one teacher must teach many students at once, whether that is a handful, tens or even hundreds in large lectures.  In larger settings, how can teachers possibly hope to clear up learners' many different misconceptions in a reasonable time?
...
\item The instructor then gives learners a multiple choice question
   that probes for misconceptions rather than simple factual recall.  A programming example is given in figure \ref{fig:mcq} which relates to integer comparison and loops.
...
\item It facilitates unintended knowledge transfer: students learn more
  ...
  instructors do things.  The extra knowledge may be high-level (e.g. whether a program is written top-down or bottom-up) or fairly low-level (e.g. using keyboard shortcuts).

\item It slows the instructor down: if the instructor has to type in the program
...
\item Watching instructors make mistakes shows learners that it's alright to make mistakes of their own~\citep{barker-live-coding}.
...
\item Instructors can spend too much time typing in boilerplate code that is needed by the lesson, but not
  ...
  all that learners take away.  This can be avoided by starting with a partial skeleton that includes the boilerplate, or having it on hand to copy and paste at the appropriate juncture.
...
learners who observe a demonstration \textit{do not} learn better than
those who did not see the demonstration~\citep{crouch-demo},
and in fact many learners misremember the outcome of demonstrations afterwards~\citep{miller-demo}.  That is, demonstrations can be useless or actively harmful to learning.
...
may seem like quite different problems to them, even though more advanced programmers would recognise them as isomorphic.
...
A principle that applies across all areas of education is that transference only comes with mastery \citep{gick-holyoak}.
...
but some learners may associate them with violence and racial or gender stereotypes, or simply find them unenjoyable.
